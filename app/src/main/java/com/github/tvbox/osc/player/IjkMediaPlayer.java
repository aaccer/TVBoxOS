...
public class IjkMediaPlayer extends IjkPlayer {
        ...
        try {
            switch (getStreamType(path)) {
                case RTSP_UDP_RTP:
                    ...
                    //mMediaPlayer.setOption(tv.danmaku.ijk.media.player.IjkMediaPlayer.OPT_CATEGORY_FORMAT, "analyzeduration", 0);
                    break;
    ...
    private int getStreamType(String path) {
        ...
        if (lowerPath.startsWith("rtsp://") || lowerPath.startsWith("udp://") || lowerPath.startsWith("rtp://") || lowerPath.contains("/udp/") || lowerPath.contains("/rtp/")) {
            return RTSP_UDP_RTP;
        }
